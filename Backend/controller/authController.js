...
function resetPassword(req, resp){
    ...
    con.query("select * from user_registration where userid=?", [userid], (err, res) => {
        if (err) {
        resp.status(403).json({message:'error in db'})
            ...
            if (res.length < 1)  //if the result array is empty
            {
                ...
                bcrypt.compare(oldPassword, res[0].password, (err, result) => {
                    ...
                    if (result) //if password match  
                    {
                    con.query("UPDATE user_registration SET password=?  where userid=?", [hash, userid], (err, res) => {
                    if (err) {
                        resp.status(407).json({ message: 'error in DB' });
                        return;
                    }
                    if (res) {
                        resp.status(200).json({ state: true,emessage: 'Password updated successfully' });
                    }
                })
            }
      })
